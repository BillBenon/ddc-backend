...
exports.create = async function (req, res) {
    ...
    if (!product) return res.status(404).send(API_RESPONSE(false, error.details[0].message, null, 400));


    const existing = await ProductOnMarket.findOne({product: req.body.product});
    if (existing) {
...
exports.update = async function (req, res) {
    ...
    const product = await Product.findById(req.body.product);
    if (!product) return res.status(404).send(API_RESPONSE(false, "Product not found", null, 500));
